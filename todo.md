...
- [x] 관리자 페이지 코드 확인
  ...
  - [x] 기존 documentUrls 호환성 유지
- [x] 통합 테스트 (실제 사용 테스트)
  - [x] AI API 키 설정 (Perplexity, Gemini, Anthropic)
  - [x] 테스트용 이미지 생성 (신원 증명 + IQ 결과지)
  - [x] S3 파일 업로드 정상 작동
  - [x] AI 검증 함수 호출 성공
  - [x] Perplexity (sonar-pro): 매우 상세한 검증 수행
  - [x] Google Gemini: 정상 작동
  - [x] Anthropic Claude: 크레딧 부족으로 비활성화
  - [x] 다중 AI 교차 검증 로직 작동 확인
  - [x] 거절 사유 상세 기록 확인
