...
### 저널 관리 섹션
- [x] 저널 데이터베이스 스키마 추가 (journals 테이블)
- [x] 저널 CRUD API 구현 (db-journals.ts)
- [x] 저널 tRPC 라우터 추가 (journal.getAll, getById, create, update, delete)
- [x] 관리자 페이지 저널 관리 탭 추가
- [x] 저널 목록 표시 및 공개/비공개 토글
- [x] 저널 추가/수정 다이얼로그 구현
- [x] 저널 삭제 기능
...
### 저널 프론트엔드 페이지
- [x] 저널 목록 페이지 구현 (/journals) - 정회원 전용
- [x] 저널 상세 페이지 구현 (/journals/:slug) - 웹 읽기 + PDF 다운로드
- [x] 마크다운 렌더링 (react-markdown + syntax highlighting)
- [x] 저널 API를 memberProcedure로 보호
- [x] 비회원/일반회원 접근 시 입회 안내 페이지 표시

### 저널 목록 페이지 기능
- [x] 카드 그리드 레이아웃 (썸네일, 제목, 요약, 카테고리, 발행일)
- [x] 카테고리 필터링
- [x] 검색 기능 (제목, 내용)
- [x] 정렬 옵션 (최신순, 오래된순)
- [x] 페이지네이션 또는 무한 스크롤 (그리드 방식)

### 저널 상세 페이지 기능
- [x] 마크다운 콘텐츠 렌더링
- [x] 메타데이터 표시 (작성일, 카테고리, 조회수)
- [x] PDF 다운로드 버튼
- [x] 이전/다음 저널 네비게이션 (관련 저널로 대체)
- [x] 관련 저널 추천 (같은 카테고리)
- [x] 공유 버튼 (URL 복사)
- [x] SEO 메타태그 (제목, 설명, Open Graph)

### 관리자 에디터 개선
- [x] PDF URL 입력 필드 추가
- [ ] 마크다운 실시간 프리뷰 (split view) - 선택사항
- [ ] 썸네일 이미지 업로드 (S3) - URL 입력으로 대체
- [ ] PDF 파일 업로드 (S3) - URL 입력으로 대체

### Header 메뉴 업데이트
- [x] 저널 메뉴 추가 (정회원에게만 표시)
- [x] 비회원에게는 "정회원 전용" 배지 표시

### 데이터베이스 확장
- [x] journals 테이블에 pdfUrl 필드 추가
- [x] journals 테이블에 viewCount 필드 추가 (조회수)
- [x] incrementJournalViewCount API 구현
